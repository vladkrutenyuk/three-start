...
- `_timer: THREE.Timer` — drives `getDeltaTime` / `getTime` / `getTimescale` / `setTimescale`. Connected to `document` and reset on `runLoop`, disconnected on `stopLoop`. Resetting on start keeps a stopped period out of the first `getDeltaTime()`; that first tick reads `performance.now()` instead of the rAF timestamp, which may predate the reset.
- `_camera`, `_canvasContainer`, `_resizeObserver`, `_isMounted`, `_isLoopRunning` — backing state for the corresponding getters / lifecycle methods.
...
| `runLoop()` | `ctx.runLoop()` | Connect timer to `document` and reset it, call `renderer.setAnimationLoop(...)`, emit `LoopRun`. |
...
Setting `ctx.camera` or `ctx.ortho` calls one private `setCamera(camera)`. Both accessors read and write the same `_camera: PerspectiveCamera | OrthographicCamera` and only cast it to their type; `ctx.isOrtho` tells which kind is actually active. `setCamera`:
...
3. Fits the camera to the container's aspect (`fitCameraToAspect`: perspective → `aspect`; orthographic → `left`/`right` around the horizontal center, keeping `top`/`bottom`) and calls `updateProjectionMatrix()`. The resize handler uses the same function.
