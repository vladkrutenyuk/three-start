...
| `ctx.camera` | The active camera, typed as `PerspectiveCamera`. Reassigning swaps the camera the render pipeline draws — fires `CameraChanged`. |
| `ctx.ortho` | The same active camera, typed as `OrthographicCamera`. Assign an orthographic camera through it. |
| `ctx.isOrtho` | `true` when the active camera is orthographic. |
...
| `ctx.render()` / `ctx.overrideRender(fn)` / `ctx.resetRender()` | Manual render control (see below). |
...
Time doesn't run while the loop is stopped: the first tick after `runLoop()` measures `getDeltaTime()` from the `runLoop()` call, not from the last tick before `stopLoop()`. Stopping the loop on idle and restarting it on demand never feeds a component one huge delta.
...
| `CameraChanged` | `newCamera, prevCamera` | `ctx.camera` or `ctx.ortho` was assigned. |
...
The default render loop calls `ctx.render()` once per frame. You rarely need to touch it, but two escape hatches exist:

```ts
// Draw a single frame on demand (synchronous):
ctx.render();
...
Call `ctx.render()` directly for editor-style UIs that don't want a continuous loop (set `manageLoopManually: true` in [`ThreeStartOptions`](/docs/api/three-start) and draw from your own dirty-tracking code). Use `overrideRender` if you've built a custom render pipeline (deferred shading, multi-pass, integrated post FX) and want three-start to drive it instead.
...
```ts
ctx.ortho = new THREE.OrthographicCamera(-5, 5, 5, -5, 0.1, 100);
ctx.ortho.zoom = 2;
ctx.ortho.updateProjectionMatrix();
```

There is one active camera; `camera` and `ortho` are two typed views of it, and both setters do the same thing. `ctx.camera` stays typed as `PerspectiveCamera` because that's the common case: no null checks or type guards on every access. The price is honesty — in an orthographic scene `ctx.camera` returns the orthographic camera. Code that must work with both kinds branches on `ctx.isOrtho`:

```ts
if (ctx.isOrtho) ctx.ortho.zoom = 2;
else ctx.camera.fov = 60;
```

`CameraChanged` listeners are typed the same way as `ctx.camera`.

On every resize an orthographic camera keeps its vertical extent (`top`/`bottom`) and horizontal center, and `left`/`right` are recomputed from the container's aspect so the image isn't stretched. A perspective camera gets its `aspect`.
